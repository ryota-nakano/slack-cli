 ...
 * Interactive chat in a channel with real-time updates
 */
...
class ChannelChatSession {
   ...
   */
  async start() {
    console.log(chalk.cyan(`🔄 #${this.channelName} の情報を取得中...\n`));
    ...
    this.messages = await this.client.getChannelHistory(this.channelId, 20);
  ...
  async checkUpdates() {
    try {
      ...
      const newMessages = await this.client.getChannelHistory(this.channelId, 100);
  ...
  async inputLoop() {
    while (true) {
      try {
        ...
        const text = await readlineInput.prompt(this.channelName, false); // isThread = false
        ...
        if (typeof text === 'object' && text.type === 'channel') {
          const newChannel = text.channel;
          this.cleanup(false);
          
          console.log(chalk.cyan(`\n📬 #${newChannel.name} に切り替えます...\n`));
          
          const newSession = new ChannelChatSession(newChannel.id, newChannel.name);
        ...
        if (trimmedText.startsWith('/history') || trimmedText.startsWith('/h')) {
  ...
  async handleDeleteMessage(msgNumber) {
    ...
    try {
      ...
      // Refresh messages
      this.messages = await this.client.getChannelHistory(this.channelId, 100);
      this.displayMessages();
    } catch (error) {
      console.error(chalk.red(`\n❌ 削除失敗: ${error.message}`));
  ...
  async sendAndDisplay(text) {
    const result = await this.client.sendMessage(this.channelId, text);
    ...
    this.client.getChannelHistory(this.channelId, 100)
      .then(newMessages => {
...
async function channelChat() {
  ...
  try {
    ...
    if (typeof result === 'object' && result.type === 'channel') {
      ...
      // Start chat session
      const session = new ChannelChatSession(selectedChannel.id, selectedChannel.name);
      await session.start();
    } else {
